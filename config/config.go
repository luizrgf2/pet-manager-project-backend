...
func init() {
	...
	} else if strings.Contains(base_path, "/cmd") {
		path_complete = strings.Split(base_path, "/cmd")[0]
	}
	...
	EXPIRATION_TIME_IN_SECONDS, err = strconv.Atoi(os.Getenv("EXPIRATION_TIME_IN_SECONDS"))
	if err != nil {
		panic(err.Error())
	}
}
